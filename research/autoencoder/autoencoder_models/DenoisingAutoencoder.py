...
class AdditiveGaussianNoiseAutoencoder(object):
    ...
    def calc_total_cost(self, X):
        """Calculate total cost on examples X."""
...
class MaskingNoiseAutoencoder(object):
    """Masking noise (randomly cover up a portion of the image)."""
    ...
    def __init__(self,
        ...
        self.hidden = self.transfer(
            tf.add(
                tf.matmul(
                    tf.nn.dropout(self.x, self.keep_prob), self.weights['w1']),
                self.weights['b1']))
        self.reconstruction = tf.add(
            tf.matmul(self.hidden, self.weights['w2']), self.weights['b2'])
